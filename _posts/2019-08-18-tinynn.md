...
- 对 layer 进行扩展：实现了 2D 卷积层、MaxPooling 层、Dropout 层以及 Tanh、SoftPlus、LeakyReLU 激活函数
- 对 losses 进行扩展：实现了 MSE，MAE，Huber 损失函数
- 对 optimizer 进行扩展：实现了 SGD、RMSProp、Momentum 等优化器，并且增加了动态调节学习率 LRScheduler
- 实现了 mnist（卷积网络和全连接网络实现分类）、nn_paint（回归）、DQN（强化学习）、VAE 和 DCGAN（无监督）等几种常见模型
...
<!--START formula-->
<div class="formula">
  $$ J_{CE}(y, \hat{y}) = -\sum_{i=1}^N\sum_{k=1}^K y_i^k \mathbb{log}(\hat{y_i^k}) $$
</div>
<!--END formula-->
...
<!--START formula-->
<div class="formula">
  $$ \hat{y}^c = \frac{\mathbb{exp}(o^c)}{\sum_{k=1}^{K} \mathbb{exp}(o^k)} $$
</div>
<!--END formula-->
...
<!--START formula-->
<div class="formula">
  $$ J_{CE} =-\sum_{i=1}^{N} (o_i^c - \mathbb{log}(\sum_{k=1}^{K} \mathbb{exp}(o_i^k))) $$
</div>
<!--END formula-->
...
<div class="formula">
  $$
  \frac{\partial J_{ce}}{\partial o^c} = -\sum_{i=1}^N(1-\frac{\mathbb{exp}(o^c)}{\sum_{k=1}^{K} \mathbb{exp}(o^k)})=\sum_{i=1}^N(\hat{y}^c-1)
  $$
</div>
<!--END formula-->
